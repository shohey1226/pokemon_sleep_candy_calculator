...
export const calcRequiredCandy = (currentLevel: number, targetLevel: number, nature: Nature, expType: ExpType) => {
...
export const calcRequiredBoostedDreamShards = (
  ...
  for (let level = currentLevel; level < targetLevel; level++) {
    ...
    carry = calcBoostedCandyExp(nature) * requiredcandy - requiredExp;
  }
  return dreamShardsSum;
};
  ...
  switch (event) {
    case "boost":
      return {
        ...
        calcRequiredExp: calcRequiredExp(formValues.currentLevel, formValues.targetLevel, formValues.expType)
      };
    case "miniBoost":
      return {
        calcRequiredCandy: calcRequiredBoostedCandy(
    ...
    default:
      return {
        calcRequiredCandy: calcRequiredCandy(
          formValues.currentLevel,
          formValues.targetLevel,
          formValues.expBoost,
          formValues.expType
        ),
        calcRequiredDreamShards: calcRequiredDreamShards(
          formValues.currentLevel,
          formValues.targetLevel,
          formValues.expBoost,
          formValues.expType
